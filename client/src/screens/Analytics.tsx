...
import { dateKey, formatMoney, todayKey } from "@/lib/format";
  ...
  const analytics = useMemo(() => {
    ...
    if (period === "month") {
      for (const e of scoped) {
        ...
        if (ymd.startsWith(ym)) currentTotal += e.amount;
        else if (ymd.startsWith(ymPrev)) prevTotal += e.amount;
      }
    } else {
      const isCurrentYear = selectedYear === localNow.getFullYear();
      const cutoffMd = isCurrentYear
        ? todayKey().slice(5) // "MM-DD"
        : "12-31";
